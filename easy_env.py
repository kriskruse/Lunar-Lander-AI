...
class Rocket(pygame.sprite.Sprite):
    ...
    def step(self, action):
        ...
        # # Update speeds
        # if self.boost and self.y < 400:
        #     self.yspeed -= 1.5
        # elif self.y > 0:
        #     self.yspeed += 1
        # if self.left and self.x < 400:
        #     self.xspeed += 2
        # if self.right and self.x > -400:
        #     self.xspeed -= 2
        if self.boost:
            if self.y >= 520:
                self.yspeed = 1
...
class LunarLander():
    ...
    def render(self):
        ...
        values = [self.rocket.y, self.rocket.x, self.rocket.yspeed, self.rocket.xspeed, self.rocket.fuel,
                  np.sqrt(400 ** 2 + 600 ** 2) / np.sqrt(self.rocket.x ** 2 + self.rocket.y ** 2)]
